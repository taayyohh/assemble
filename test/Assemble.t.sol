...
contract AssembleTest is Test {
    ...
    function test_EventCreationValidation() public {
        ...
        splits[0] = Assemble.PaymentSplit(alice, 10000, "organizer");
    ...
    function test_TokenIdGeneration() public {
        uint256 tokenId = assemble._generateTokenId(
            Assemble.TokenType.EVENT_TICKET,
            1,
            0,
            1
        );
        ...
        assertTrue(assemble._isValidTicketForEvent(tokenId, 1));
        assertFalse(assemble._isValidTicketForEvent(tokenId, 2));
    }
    ...
    function test_PurchaseTickets() public {
        ...
        uint256 protocolFee = (0.1 ether * 50) / 10000; // 0.5%
        uint256 netAmount = 0.1 ether - protocolFee;
        uint256 aliceShare = (netAmount * 7000) / 10000; // 70%
        uint256 bobShare = (netAmount * 3000) / 10000;   // 30%
    ...
    function test_TipEvent() public {
        ...
        uint256 protocolFee = (0.5 ether * 50) / 10000; // 0.5%
    ...
    function test_CheckInAndAttendanceBadge() public {
        ...
        uint256 ticketId = assemble._generateTokenId(
            Assemble.TokenType.EVENT_TICKET,
            eventId,
            0,
            1
        ...
        uint256 badgeId = assemble._generateTokenId(
            Assemble.TokenType.ATTENDANCE_BADGE,
            eventId,
            0,
            0
        );
    ...
    function test_CheckInFailsWithoutTicket() public {
        ...
        uint256 fakeTicketId = assemble._generateTokenId(
            Assemble.TokenType.EVENT_TICKET,
            eventId,
            0,
            1
    ...
    function test_OrganizerCredential() public {
        ...
        uint256 credId = assemble._generateTokenId(
            Assemble.TokenType.ORGANIZER_CRED,
            eventId,
            0,
            0
        );
    ...
    function test_GroupDiscountCalculation() public {
        ...
        assertEq(assemble._calculateGroupDiscount(2), 0);      // No discount for 2
        assertEq(assemble._calculateGroupDiscount(3), 250);    // 2.5% for 3-4
        assertEq(assemble._calculateGroupDiscount(5), 500);    // 5% for 5-9
        assertEq(assemble._calculateGroupDiscount(10), 1000);  // 10% for 10+
    }
